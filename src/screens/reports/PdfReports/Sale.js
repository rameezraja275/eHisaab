...
const Bill = (props) => {
    ...
    const { saleReport, bussiness, navigation } = props
    const { date, total_sale, total_discount, total_paid } = navigation.state.params

    let htmlTable = "";
    for (let i = 0; i < saleReport.length; i++) {
        const item = saleReport[i];
        htmlTable += `<tr>

        <td > ${item.sale_date} </td>
        <td > ${item.customer_name ? item.customer_name : ""} </td>
        <td > ${FormatPrice(item.sale_amount)} </td>
        <td > ${FormatPrice(item.sale_discount)} </td>
    ...
    <style> body { font-family: "Noto Sans", sans-serif; font-size: 10px; display: flex; justify-content: center; }
        ...
        bussiness.phone ? bussiness.phone : ""
                ...
                <div class="width40"> 
                    ...
                    <div class="flex mb-5">
                        <span class="flex1"> Total Sale : </span> 
                        <span> ${FormatPrice(total_sale)} </span> 
                    </div>
